  ...
  return (
    <div className="flex flex-wrap items-center gap-2 border-b border-border/80 bg-surface-1/50 px-3 py-2.5">
      <div className="min-w-0 flex-1 basis-[min(100%,12rem)]">
        <p className="truncate font-mono text-[0.8125rem] text-dim-foreground" title={cwd}>
          {cwd}
        </p>
        <div className="mt-1 flex flex-wrap items-center gap-2">
          {fileCount > 0 && (
            <span className="rounded-md bg-surface-2 px-2 py-0.5 font-mono text-[0.8125rem] text-muted-foreground">
              {fileCount} file{fileCount !== 1 ? 's' : ''}
            </span>
          )}
          {(stats.additions > 0 || stats.deletions > 0) && (
            <>
              <span className="inline-flex items-center gap-1 rounded-md bg-emerald-500/15 px-2 py-0.5 font-mono text-[0.8125rem] font-medium text-emerald-300 light:text-emerald-800">
                +{stats.additions}
              </span>
              <span className="inline-flex items-center gap-1 rounded-md bg-rose-500/15 px-2 py-0.5 font-mono text-[0.8125rem] font-medium text-rose-300 light:text-rose-800">
                −{stats.deletions}
              </span>
            </>
          )}
      ...
      <div className="flex shrink-0 items-center gap-1.5">
        ...
        <button
          ...
          onClick={onRefresh}
          disabled={loading}
          className="rounded-lg border border-border bg-surface-0 px-3 py-1.5 text-[0.875rem] font-medium text-foreground shadow-sm transition-colors hover:bg-surface-2 disabled:opacity-45"
        >
          {loading ? 'Loading…' : 'Refresh'}
        </button>
      </div>
      {truncated && (
        <p className="w-full text-[0.8125rem] text-warning">
...
function FeedbackBlock({
  ...
  return (
    ...
    >
      <label className="text-[0.8125rem] font-semibold uppercase tracking-[0.12em] text-dim-foreground">
      ...
      <textarea
        ...
        disabled={busy}
        rows={compact ? 3 : 4}
        className="w-full resize-y rounded-xl border border-border bg-surface-1 px-3 py-2.5 text-[1rem] text-foreground placeholder:text-dim-foreground shadow-inner outline-none transition-shadow focus:ring-2 focus:ring-primary/35 disabled:opacity-50"
      />
      <button
        type="button"
        onClick={onSend}
        disabled={!canSend}
        className="w-full rounded-xl bg-primary py-2.5 text-[1rem] font-semibold text-primary-foreground shadow-md transition-opacity hover:opacity-92 disabled:pointer-events-none disabled:opacity-35"
      >
        {sending ? 'Sending…' : 'Send feedback to agent'}
      </button>
      {busy && (
        <p className="text-center text-[0.875rem] text-dim-foreground">Disabled while the agent is running.</p>
      )}
    </div>
  )
}
...
function DiffPanelImpl({ sessionId, cwd, status, onSendFeedback }: DiffPanelProps) {
  ...
  const expandBtn = (
    <button
      ...
      className="inline-flex items-center gap-1.5 rounded-lg border border-border-strong bg-surface-2 px-2.5 py-1.5 text-[0.875rem] font-medium text-foreground transition-colors hover:bg-surface-3"
  ...
  return (
    <>
      <div className="flex h-full min-h-[220px] flex-col overflow-hidden rounded-xl border border-border bg-linear-to-b from-surface-1/40 to-surface-0/30 shadow-sm">
        ...
        {errorMessage && (
          <p className={cn('px-3 py-2 text-[0.9375rem]', loadError ? 'text-destructive' : 'text-warning')}>
            {errorMessage}
          </p>
        )}
          ...
          <div ref={compactScrollRef} className="min-h-0 flex-1 overflow-auto">
            {isEmpty && (
              <div className="flex flex-col items-center justify-center gap-2 px-4 py-10 text-center">
                <div className="rounded-2xl border border-border bg-surface-1/50 px-4 py-3 font-mono text-[0.875rem] text-muted-foreground">
                  Working tree clean
                </div>
                <p className="max-w-56 text-[0.875rem] text-dim-foreground">No unstaged changes in this workspace.</p>
              </div>
            )}
            ...
            <div className="flex min-h-0 flex-1 overflow-hidden bg-muted/25">
              ...
              {isEmpty && !errorMessage && (
                <div className="flex flex-1 flex-col items-center justify-center gap-3 p-16 text-muted-foreground">
                  ...
                  <p className="text-[0.9375rem] text-dim-foreground">Refresh after the agent edits files.</p>
                </div>
              )}
            </div>
    ...
    <div className="shrink-0 border-b border-border bg-surface-1/95 px-5 pb-4 pt-2 backdrop-blur-md md:px-10">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 max-w-[min(100vw-3rem,48rem)] truncate font-mono text-[0.9375rem] text-dim-foreground" title={cwd}>
            {cwd}
          </p>
          <div className="mt-3 flex flex-wrap items-center gap-2">
            {fileCount > 0 && (
              <span className="rounded-lg bg-surface-2 px-3 py-1 font-mono text-[0.9375rem] text-muted-foreground">
                {fileCount} file{fileCount !== 1 ? 's' : ''}
              </span>
            )}
            {(stats.additions > 0 || stats.deletions > 0) && (
              <>
                <span className="rounded-lg bg-emerald-500/15 px-3 py-1 font-mono text-[0.9375rem] font-semibold text-emerald-300 light:text-emerald-800">
                  +{stats.additions} additions
                </span>
                <span className="rounded-lg bg-rose-500/15 px-3 py-1 font-mono text-[0.9375rem] font-semibold text-rose-300 light:text-rose-800">
      ...
      {truncated && (
        <p className="mt-3 text-[0.9375rem] text-warning">This diff was truncated on the server.</p>
      )}
    </div>
  )
}
