...
function makeCtx(toolName: string, args: Record<string, unknown>): ToolExecutionContext & { denied?: string } {
  ...
  return {
    ...
    loop: { messages: [], iteration: 0, maxIterations: 10, sessionId: 'test', usage: { inputTokens: 0, outputTokens: 0 }, lastUsage: undefined, resumed: false, elapsedMs: 0, stop: () => {}, signal: ac.signal, logger },
...
describe('permissions middleware', () => {
  ...
  it('denied tool call becomes error result in loop', async () => {
    const { AgentLoop, ToolRegistry } = await import('@chinmaymk/ra')
