...
```yaml
thinking: high  # low (1k tokens) | medium (8k) | high (32k)
```
...
> Bedrock falls back to the standard AWS credential chain. Azure falls back to `DefaultAzureCredential`. All providers support custom base URLs via `--<provider>-base-url` flags.
...
15 tools enabled by default. The agent can read, write, search, execute, and interact out of the box.
