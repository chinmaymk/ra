...
describe('AgentLoop', () => {
  ...
  it('runs middleware at lifecycle points', async () => {
    ...
    const loop = new AgentLoop({
      ...
      maxIterations: 10,
      middleware: {
        beforeLoopBegin: [async (_ctx, next) => { events.push('beforeLoopBegin'); await next() }],
        beforeModelCall: [async (_ctx, next) => { events.push('beforeModelCall'); await next() }],
        afterModelResponse: [async (_ctx, next) => { events.push('afterModelResponse'); await next() }],
        afterLoopIteration: [async (_ctx, next) => { events.push('afterLoopIteration'); await next() }],
        afterLoopComplete: [async (_ctx, next) => { events.push('afterLoopComplete'); await next() }],
        onStreamChunk: [],
        beforeToolExecution: [],
        afterToolExecution: [],
        onError: [],
